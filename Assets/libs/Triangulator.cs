...
//namespace DelauneyTriangulator
//{
	/// <summary>
	...
	/// </remarks>
	public static class Triangulator// : MonoBehaviour 
	{
		
		public static int[] ConvertToVertexIndexList( List<Triangle> triangleList){
				
			int[] newTriangles = new int[triangleList.Count*3];
			
			for( int i = 0; i < triangleList.Count; i++){
				
				newTriangles[i*3+0] = triangleList[i].p1;
				newTriangles[i*3+1] = triangleList[i].p2;
				newTriangles[i*3+2] = triangleList[i].p3;
			}
			return newTriangles;
		}
		
		public static List<Triangle> Triangulate( Vector3[] vertices)
		{			
			List<Vector2> verticesList = new List<Vector2>();
			for( int i = 0; i < vertices.Length; i++ )
				verticesList.Add( new Vector2(vertices[i].x, vertices[i].z) );
			
			return Triangulate( verticesList);
		}
		...
		/// method:
		/// <code>
		/// if(!Vertices.Exists(delegate(Vector2 p) { return pNew.Equals2D(p); }))
		///		Vertices.Add(pNew);
		/// </code>
		/// </para>
